...
public class CompareAndOutput1{
	public static String compareHeaders(String actualCount, String expectedCount, String actualOrder,
		...
		if (!actualOrder.replaceAll("[\\[\\]]", "").trim().equals(expectedOrder.trim())) {
			comments.append("The expected header levels are [").append(expectedOrder)
					.append("], but the actual header levels are [")
					.append(actualOrder.replaceAll("[\\[\\]]", "").trim()).append("]. ");
		}

		// Compare header names
		List<String> actualHeaderList = Arrays.asList(actualNames.split(",\\s*"));
		List<String> expectedHeaderList = Arrays.asList(expectedNames.split(",\\s*"));
		List<String> missingLevels = new ArrayList<>();

		for (int i = 0; i < expectedHeaderList.size(); i++) {
			String expectedName = expectedHeaderList.get(i).trim();
			if (i < actualHeaderList.size()) {
				String actualName = actualHeaderList.get(i).trim();
		...
		return comments.length() == 0 ? "No discrepancies found." : comments.toString();
	}
	...
	private static String getExpectedLevel(String expectedOrder, int index) {
		String[] levels = expectedOrder.replaceAll("[\\[\\]]", "").split(",\\s*");

...
public class HeaderComparison {
		...
		if (!actualOrderList.equals(expectedOrderList)) {
			...
			for (int i = 0; i < expectedHeaders.size(); i++) {
				if (i < actualHeaders.size()) {
					...
					if (!actualHeader.contains(expectedLevel)) {
						message.append("Header \"").append(expectedHeader).append("\" is expected to be at level ")
								.append(expectedLevel).append(", but found in header: \"").append(actualHeader)
								.append("\".\n");
					}
				}
			}
	...
	// Utility function to normalize input by handling comma and newline-separated
	// data
